...
```{r}
raster_sample_5 <- rasters[[5]]

raster_sample_5 <- as.integer(raster_sample) # should round this TODO

writeRaster(raster_sample_5, "sample_data/sample_frame_raster_2.tif", format = "GTiff")

#ras %>% as.matrix() 
# all unique valeues! how can make bands like this O.O
```

WRITE ALL RASTERS TO TIF FILES
```{r}
for(i in 1:nrow(date_frames)) {
  sample_raster <- rasters[[i]]
  sample_raster <- as.integer(sample_raster)
  filename <- paste0("sample_data/sample_frame_tifs/sample_frame_raster_", i)
  writeRaster(sample_raster, filename, format = "GTiff")
}
```
